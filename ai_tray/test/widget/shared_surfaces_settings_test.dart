...
void main() {
  testWidgets('settings advanced shows Copilot enablement and diagnostics', (
    tester,
  ) async {
    ...
    expect(find.text('GITHUB COPILOT (EXPERIMENTAL)'), findsOneWidget);
    expect(find.text('Enable GitHub Copilot'), findsOneWidget);
    expect(find.text('SDK / CLI version'), findsOneWidget);
    expect(find.text('1.0.7 / —'), findsOneWidget);
    expect(find.text('Authentication'), findsOneWidget);
    expect(find.text('Authenticated'), findsOneWidget);
    expect(find.text('Experimental API'), findsOneWidget);
    expect(find.text('Open Copilot settings'), findsOneWidget);

    await tester.tap(
      find.widgetWithText(SwitchListTile, 'Enable GitHub Copilot'),
    );
    await tester.pumpAndSettle();
    expect((await settings.read()).copilotEnabled, isFalse);
    expect(find.textContaining('Copilot is disabled'), findsOneWidget);
  });
}
