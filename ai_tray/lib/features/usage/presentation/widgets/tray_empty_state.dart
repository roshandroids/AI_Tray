  ...
  static ({String title, String body, String? hint}) _copyFor(
    ...
    if (failure == null) {
      return (
        ...
        body: 'Press Refresh to fetch the latest session and weekly limits '
            'from Claude Code.',
    ...
    return switch (failure.code) {
      ...
      FailureCode.processNonZeroExit => (
          ...
          body: failure.message,
          hint:
              "Verify `claude -p '/usage' --output-format json` in Terminal.",
        ),
      FailureCode.parserFailure ||
      FailureCode.unknownCliOutput ||
      FailureCode.incompleteOutput => (
          title: 'Unexpected Claude output',
          body: 'Limits were not present in this response. Cached values are '
