...
| D-027 | 2026-08-02 | Pin `platform-ci` workflow calls to a resolved commit SHA (`ae93a81f35e088ccd8026557d27cb95e24b3fd12`, resolved from tag `v1`) instead of the mutable `@v1` tag, across `quality.yml`, `release-pr.yml`, and `release.yml` | Active; closes the D-023 follow-up item — a future change to `platform-ci`'s `v1` tag can no longer silently alter this repo's release behavior; bump deliberately by resolving a new SHA |
